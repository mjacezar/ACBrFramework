...
public final class ACBrECF extends ACBrClass {
    ...
    /**
     * Informacoes relativas ao rodape dos cupons.
     */
    ...
    /**
     * Manipulacao da criptografia.
     */
    private ACBrAAC aac = null;
    /**
     * Manipulacao da assinaturas EAD.
     */
    ...
    public void setAAC(ACBrAAC pAac) throws ACBrException {
        ...
        int ret = ACBrECFInterop.INSTANCE.ECF_SetAAC(getHandle(), lHandle);
        checkResult(ret);
        this.aac = pAac;
    }       
    ...
    public void setEAD(ACBrEAD pEad) throws ACBrException {
        ...
        int ret = ACBrECFInterop.INSTANCE.ECF_SetEAD(getHandle(), lHandle);
        checkResult(ret);
        this.ead = pEad;
    }
    ...
    public Date getDataHoraUltimaReducaoZ() throws ACBrException {
        DoubleByReference lReturnValue = new DoubleByReference();
        int ret = ACBrECFInterop.INSTANCE.ECF_GetDataHoraUltimaReducaoZ(getHandle(), lReturnValue);
        checkResult(ret);
        return OleDate.fromOADate(lReturnValue.getValue());
    }
    ...
    /**
     * Obtem as informacoes do consumidor.
     * @return
     * @throws ACBrException 
     */
    ...
    public String dadosUltimaReducaoZ() throws ACBrException {
        final int BUFFER_LEN = 16384;
        ByteBuffer returnBuffer = ByteBuffer.allocate(BUFFER_LEN);
        int ret = ACBrECFInterop.INSTANCE.ECF_GetDadosUltimaReducaoZ(getHandle(), returnBuffer, BUFFER_LEN);
        checkResult(ret);
    ...
    public String dadosReducaoZ() throws ACBrException {
        final int BUFFER_LEN = 16384;
        ByteBuffer returnBuffer = ByteBuffer.allocate(BUFFER_LEN);
        int ret = ACBrECFInterop.INSTANCE.ECF_GetDadosReducaoZ(getHandle(), returnBuffer, BUFFER_LEN);
        checkResult(ret);
    ...
    public void identificaPAF(String nomeVersao, String md5) throws ACBrException, ACBrException, ACBrException {
    ...
    public void pafMF_RelMeiosPagamento(FormaPagamento[] pFormasPagamento,
        ...
        int ret = ACBrECFInterop.INSTANCE.ECF_PafMF_RelMeiosPagamento(getHandle(),
                lFormasPagamento, pFormasPagamento.length, pTitulo, pIndiceRelatorio);
        checkResult(ret);
    }
    ...
    public void pafMF_RelDAVEmitidos(DAV[] pDAVs, String pTituloRelatorio, int pIndiceRelatorio) throws ACBrException {
        if (pDAVs == null) throw new ACBrException("Nenhuma DAV informada.");
        ACBrECFInterop.DAVsRec[] lRecord = criaVetorJNA(ACBrECFInterop.DAVsRec.class, pDAVs.length);
        for (int i = 0; i < pDAVs.length; i++) {
            lRecord[i].Numero = toByte(pDAVs[i].getNumero(), 14);
            lRecord[i].COO_Cupom = pDAVs[i].getCooCupom();
            lRecord[i].COO_Dav = pDAVs[i].getCooDAV();
            lRecord[i].Titulo = toByte(pDAVs[i].getTitulo(), 31);
            lRecord[i].DtEmissao = OleDate.toOADate(pDAVs[i].getDtEmissao());
            lRecord[i].Valor = pDAVs[i].getValor();
        }

        int ret = ACBrECFInterop.INSTANCE.ECF_PafMF_RelDAVEmitidos(getHandle(), lRecord, pDAVs.length, 
                toUTF8(pTituloRelatorio), pIndiceRelatorio);
        checkResult(ret);
    }    
    
    public void pafMF_RelIdentificacaoPafECF(IdenticacaoPaf pIdentificacaoPAF, int pIndiceRelatorio) throws ACBrException {
        IdenticacaoPaf lIdentificaPAF;

        if (pIdentificacaoPAF == null) lIdentificaPAF = getAAC().getIdentPaf();
        else lIdentificaPAF = pIdentificacaoPAF;

        int ret = ACBrECFInterop.INSTANCE.ECF_PafMF_RelIdentificacaoPafECF(getHandle(), lIdentificaPAF.getHandle(), pIndiceRelatorio);
        checkResult(ret);
    }    
    
    public void pafMF_RelParametrosConfiguracao(InfoPaf pInfoPAF, int pIndiceRelatorio) throws ACBrException {
        InfoPaf lInfo;

        if (pInfoPAF == null) lInfo = getAAC().getIdentPaf().getPaf();
        else lInfo = pInfoPAF;

        int ret = ACBrECFInterop.INSTANCE.ECF_PafMF_RelParametrosConfiguracao(getHandle(), 
                lInfo.getHandle(), pIndiceRelatorio);
        checkResult(ret);
    }
     
    public void pafMF_RelParametrosConfiguracao(String pPerfil, int pIndiceRelatorio) throws ACBrException {
        int ret = ACBrECFInterop.INSTANCE.ECF_PafMF_RelParametrosConfiguracao2(getHandle(), toUTF8(pPerfil), pIndiceRelatorio);
        checkResult(ret);
    }    
